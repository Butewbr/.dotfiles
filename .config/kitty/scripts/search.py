...
class Search(Handler):
    ...
    def on_key(self, key_event: KeyEventType) -> None:
        ...
        if key_event.matches("ctrl+u"):
            ...
            self.switch_mode()
            self.refresh()
        elif key_event.matches("up") or key_event.matches("shift+enter"):
            for match_arg in self.match_args():
                call_remote_control(["kitten", match_arg, str(SCROLLMARK_FILE)])
        elif key_event.matches("down") or key_event.matches("enter"):
            for match_arg in self.match_args():
                call_remote_control(["kitten", match_arg, str(SCROLLMARK_FILE), "next"])
        elif key_event.matches("ctrl+enter") or key_event.matches("esc"):
            self.quit(0)
        elif key_event.matches("ctrl+c"):
            self.quit(1)
